#!/usr/bin/env python3
"""
QIS v6.0 - Cross-Platform Zero Flash Launcher
...
def launch_qis_hidden():
    ...
    try:
        ...
        qis_path = os.path.join(script_dir, 'qis_v6.py')
        
        # Check if QIS file exists
        if not os.path.exists(qis_path):
            messagebox.showerror("QIS v6.0 - File Not Found", 
                               f"Could not find qis_v6.py in:\n{script_dir}\n\n"
        ...
        if system_name == 'windows':
            ...
            # Linux/macOS: Standard hidden launch
            subprocess.Popen([sys.executable, qis_path], 
        ...
        messagebox.showerror("QIS v6.0 - Launch Error", 
...
def main():
    ...
    try:
        ...
        messagebox.showerror("QIS v6.0 - Unexpected Error", 
                           f"Unexpected error:\n\n{str(e)}")
    finally:
        # Clean exit
        root.destroy()
