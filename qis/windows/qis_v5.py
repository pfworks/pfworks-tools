#!/usr/bin/env python3
"""
HAL 9000 - Amazon Q Interface for Windows
Windows-specific version with WSL detection and proper handling
Includes all features from the Linux/macOS version plus Windows-specific enhancements
"""
...
class WindowsEnvironmentDetector:
    """Detect and handle Windows environment specifics including WSL"""
    
    def __init__(self):
        self.is_windows = platform.system().lower() == 'windows'
        self.is_wsl = self._detect_wsl()
        self.wsl_distro = self._get_wsl_distro() if self.is_wsl else None
    ...
    def _find_q_cli(self):
        """Find Q CLI executable in various locations"""
        possible_paths = []
        ...
        if self.is_windows:
            ...
            # WSL paths - check both Linux and Windows paths
            possible_paths.extend([
                'q',
                '/usr/local/bin/q',
                '/usr/bin/q',
                # Windows paths accessible from WSL
                '/mnt/c/Program Files/Amazon Q/q.exe',
                '/mnt/c/Program Files (x86)/Amazon Q/q.exe'
            ])
        else:
...
class WindowsQService:
    """Windows-specific Q service with WSL handling and SSH routing"""
    
    def __init__(self):
        self.env_detector = WindowsEnvironmentDetector()
        self.env_info = self.env_detector.get_environment_info()
        
        # Initialize SSH Q service
        self.ssh_q_service = SSHQService()
        
        # Q CLI method selection: "local", "wsl", "ssh"
        self.q_method = "auto"  # auto, local, wsl, ssh
    ...
    def get_q_method_status(self):
        ...
        # Check WSL availability more thoroughly
        wsl_available = False
        try:
            wsl_available = self._check_wsl_q_cli()
        except Exception as e:
            wsl_available = False
        ...
        methods = {
            'local': {
                ...
                'name': 'Local Q CLI',
                'description': 'Windows native Q CLI'
            },
            'wsl': {
                'available': wsl_available,
                'name': 'WSL Q CLI', 
                'description': 'Q CLI in WSL environment'
            },
            'ssh': {
    ...
    def query(self, question, context=None):
        ...
        if self.q_method == "ssh":
            ...
            try:
                ...
                if hasattr(subprocess, 'run'):
                    result = subprocess.run([q_path, 'chat', question],
                                          input='y\n',
                                          capture_output=True,
                                          text=True,
                                          timeout=30,
                    ...
                    proc = subprocess.Popen([q_path, 'chat', question],
    ...
    def get_status(self):
        ...
        if self.q_method == "ssh":
            ...
            return {
                'available': self.env_info['q_cli_available'],
                'method': 'Local Windows Q CLI' if self.env_info['q_cli_available'] else None,
                'environment': self.env_info,
                'q_method': self.q_method
            }
...
class HALWindowsInterface:
    ...
    def __init__(self, root):
        ...
        self.current_mode = "Q"  # "Q" for Q CLI, "POWERSHELL" for commands
        ...
        # Initialize Windows Q service
        self.q_service = WindowsQService()
    ...
    def show_unified_settings(self):
        ...
        try:
            ...
            if not hasattr(self, '_settings_dialog'):
                self._settings_dialog = UnifiedSettingsDialog(self.root)
    ...
    def apply_skin(self, skin_manager):
        ...
        try:
            ...
            if identity:
                ...
                personality_name = identity.get('name', 'QIS v5.0')
                new_title = personality_name + " - QIS v5.0"
    ...
    def update_system_status(self):
        """Update system status based on Q service availability and Windows environment"""
        try:
    ...
    def update_connection_status(self):
        ...
        try:
            if self.current_mode == "Q":
                ...
                if env_info['is_wsl']:
                    self.connection_status.config(text="POWERSHELL: READY (WSL)")
                elif env_info['is_windows']:
                    self.connection_status.config(text="POWERSHELL: READY (Windows)")
                else:
                    self.connection_status.config(text="POWERSHELL: READY")
    ...
    def setup_control_buttons(self, parent):
        ...
        self.shell_mode_btn = ttk.Button(mode_frame, text="POWERSHELL", 
                                        command=lambda: self.set_mode("POWERSHELL"), 
                                        style='HAL.TButton')
...
#                   command=self.configure_ssh, style='HAL.TButton').pack(side=tk.LEFT, padx=1)
    ...
    def set_mode(self, mode):
        """Switch between Q CLI and PowerShell modes"""
        ...
        if mode == "Q":
            ...
            self.input_label.config(text="POWERSHELL INPUT:")
        
        # Update connection status when mode changes
        self.update_connection_status()
        
        # Add mode switch message
        mode_name = "Q CLI" if mode == "Q" else "PowerShell Command"
        self.add_message("SYSTEM", f"Switched to {mode_name} mode", "system")
    
    def handle_tab_completion(self, event):
        """Handle tab completion for powershell mode (Windows-specific)"""
        if self.current_mode != "POWERSHELL":
...
- POWERSHELL: Execute Windows commands with retro styling
...
I am fully operational and ready to assist you with AWS-related queries and PowerShell commands.
...
Available modes:
- Q CLI: Direct interaction with Amazon Q for AWS assistance
- POWERSHELL: Execute Windows commands and view output

To switch modes, click the Q CLI or POWERSHELL buttons above.
...
My mission is to provide you with accurate information and help you accomplish your objectives."""
    ...
    def send_shell_command(self, command):
        """Execute PowerShell command (Windows-specific)"""
        # Add command to display
        self.add_message("POWERSHELL", f"{self.shell_cwd}> {command}", "powershell_user")
        
        # Update status
        env_info = self.q_service.env_info
        if env_info['is_wsl']:
            self.connection_status.config(text="POWERSHELL: EXECUTING (WSL)...")
        else:
            self.connection_status.config(text="POWERSHELL: EXECUTING (Windows)...")
    ...
    def process_shell_command(self, command):
        """Process PowerShell command with Windows/WSL handling"""
        try:
            env_info = self.q_service.env_info
            ...
            if command.strip().lower().startswith('cd '):
                ...
                try:
                    ...
                    if os.path.isdir(new_cwd):
                        ...
                        self.output_queue.put(('powershell_success', f"Changed directory to: {self.shell_cwd}"))
                        self.output_queue.put(('update_mode_label', None))
                    else:
                        self.output_queue.put(('powershell_error', f"cd: {path}: No such file or directory"))
                except Exception as e:
                    self.output_queue.put(('powershell_error', f"cd: {str(e)}"))
                return
            
            # Execute other commands with Windows/WSL-specific handling
            if env_info['is_wsl']:
            ...
            # Send output
            if result.stdout:
                self.output_queue.put(('powershell_output', result.stdout.strip()))
            
            if result.stderr:
                self.output_queue.put(('powershell_error', result.stderr.strip()))
            
            if result.returncode != 0 and not result.stderr:
                self.output_queue.put(('powershell_error', f"Command exited with code {result.returncode}"))
                
        except subprocess.TimeoutExpired:
            self.output_queue.put(('powershell_error', 'Command timed out'))
        except Exception as e:
            self.output_queue.put(('powershell_error', f'Error: {str(e)}'))
...
• PowerShell command execution (Windows/WSL)
